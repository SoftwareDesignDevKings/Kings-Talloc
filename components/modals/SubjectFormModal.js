    ...
    showModal && (
      <div className="tw-fixed tw-inset-0 tw-flex tw-items-center tw-justify-center tw-bg-black tw-bg-opacity-50 tw-z-50">
        <div className="tw-bg-white tw-rounded-lg tw-shadow-lg tw-w-full tw-max-w-md tw-p-6 tw-z-60">
          <h2 className="tw-text-2xl tw-font-bold tw-text-center">{subject ? 'Edit Subject' : 'Add Subject'}</h2>
          <form onSubmit={handleFormSubmit} className="tw-space-y-4 tw-mt-4">
            <div>
              <label className="tw-block tw-text-sm tw-font-medium tw-text-gray-700">Subject Name</label>
              <input
                type="text"
                value={subjectName}
