 ...
 */
export const createTeamsMeeting = async (subject, description, startTime, endTime, attendeesEmailArr) => {
	try {
		...
		if (!response.ok) {
			...
			throw new Error(`Failed to create Teams meeting: ${error.error?.message || response.statusText}`);
		}
...
export const updateTeamsMeeting = async (eventId, subject, description, startTime, endTime, attendeesEmailArr) => {
	try {
		const session = await getSession();
		const accessToken = session?.user?.microsoftAccessToken;
		...
		if (!response.ok) {
			const error = await response.json();
			throw new Error(`Failed to update Teams meeting: ${error.error?.message || response.statusText}`);
		}
...
export const deleteTeamsMeeting = async (eventId) => {
	try {
		...
		if (!response.ok && response.status !== 404) {
			const error = await response.json();
			throw new Error(`Failed to delete Teams meeting: ${error.error?.message || response.statusText}`);
		}
