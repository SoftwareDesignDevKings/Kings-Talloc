                ...
                {events.length === 0 ? (
                    ...
                    <div className="list-group list-group-flush">
                        {events.map((event, index) => (
                            <div
                                key={event.id}
                                className={`list-group-item border-0 px-0 ${index === 0 ? 'pt-0 pb-3' : 'py-3'}`}
                            >
                                <div className="d-flex align-items-start">
                                    <div className="flex-shrink-0 me-3">
                                        <div
                                            className={`${isToday ? 'bg-success' : 'bg-primary'} bg-opacity-10 rounded p-3 text-center`}
                                            style={{ minWidth: '70px' }}
                                        >
                                            <div
                                                className={`fw-semibold ${isToday ? 'text-success' : 'text-primary'}`}
                                            >
                                                {isToday
                                                    ? 'Today'
