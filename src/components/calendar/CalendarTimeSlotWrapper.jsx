...
const CustomTimeSlotWrapper = ({
    ...
    const filteredAvailabilities =
        selectedTutors.length > 0
            ? applicableAvailabilities.filter((availability) =>
    ...
    const isFullyAvailable = (availability) => {
        ...
        return (
            (availStart <= slotStart || availStart.getTime() === slotStart.getTime()) &&
    ...
    const backgroundColor =
        availableTutors.length > 0
            ? calculateGreenIntensity(availableTutors.length, maxTutors)
            : 'transparent';
